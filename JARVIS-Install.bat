...
echo f.WriteLine "$OLLAMA_EXE  = 'https://github.com/ollama/ollama/releases/latest/download/OllamaSetup.exe'">>"%VBSHELPER%"
...
echo f.WriteLine "function Step { param($n,$m) Write-Host ""  [$n/7] $m"" -ForegroundColor Cyan }">>"%VBSHELPER%"
...
echo f.WriteLine "    } catch { FAIL ('Clone failed: ' + $_.Exception.Message) }">>"%VBSHELPER%"
echo f.WriteLine "}">>"%VBSHELPER%"
...
echo f.WriteLine "Step 5 'Installing Ollama + Llama 3.2 (local AI, free, no internet needed)...'">>"%VBSHELPER%"
echo f.WriteLine "$ollamaCmd = Get-Command ollama -ErrorAction SilentlyContinue">>"%VBSHELPER%"
echo f.WriteLine "if (-not $ollamaCmd) {">>"%VBSHELPER%"
echo f.WriteLine "    WARN 'Ollama not found. Downloading Ollama (~50 MB)...'">>"%VBSHELPER%"
echo f.WriteLine "    $ollamaTmp = Join-Path $env:TEMP 'OllamaSetup.exe'">>"%VBSHELPER%"
echo f.WriteLine "    try {">>"%VBSHELPER%"
echo f.WriteLine "        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12">>"%VBSHELPER%"
echo f.WriteLine "        INFO 'Downloading Ollama installer...'">>"%VBSHELPER%"
echo f.WriteLine "        (New-Object System.Net.WebClient).DownloadFile($OLLAMA_EXE, $ollamaTmp)">>"%VBSHELPER%"
echo f.WriteLine "        INFO 'Installing Ollama silently (this may take a minute)...'">>"%VBSHELPER%"
echo f.WriteLine "        Start-Process $ollamaTmp -ArgumentList '/S' -Wait">>"%VBSHELPER%"
echo f.WriteLine "        RefreshPath">>"%VBSHELPER%"
echo f.WriteLine "        Start-Sleep -Seconds 3">>"%VBSHELPER%"
echo f.WriteLine "        $ollamaCmd = Get-Command ollama -ErrorAction SilentlyContinue">>"%VBSHELPER%"
echo f.WriteLine "        if ($ollamaCmd) { OK 'Ollama installed!' }">>"%VBSHELPER%"
echo f.WriteLine "        else { WARN 'Ollama install may need a restart. Continuing...' }">>"%VBSHELPER%"
echo f.WriteLine "    } catch { WARN ('Ollama download failed: ' + $_.Exception.Message + '. You can install it later from https://ollama.com') }">>"%VBSHELPER%"
echo f.WriteLine "} else { OK ('Ollama found: ' + (ollama --version 2^>^&1)) }">>"%VBSHELPER%"
echo f.WriteLine "# Start ollama serve in background so we can pull the model">>"%VBSHELPER%"
echo f.WriteLine "$ollamaCmd2 = Get-Command ollama -ErrorAction SilentlyContinue">>"%VBSHELPER%"
echo f.WriteLine "if ($ollamaCmd2) {">>"%VBSHELPER%"
echo f.WriteLine "    INFO 'Starting Ollama service...'">>"%VBSHELPER%"
echo f.WriteLine "    $serveJob = Start-Process ollama -ArgumentList 'serve' -WindowStyle Hidden -PassThru -ErrorAction SilentlyContinue">>"%VBSHELPER%"
echo f.WriteLine "    Start-Sleep -Seconds 4">>"%VBSHELPER%"
echo f.WriteLine "    # Check if llama3.2 is already pulled">>"%VBSHELPER%"
echo f.WriteLine "    $modelList = (ollama list 2^>^&1) -join ' '">>"%VBSHELPER%"
echo f.WriteLine "    if ($modelList -notmatch 'llama3.2') {">>"%VBSHELPER%"
echo f.WriteLine "        WARN 'Downloading Llama 3.2 AI model (~2 GB). This is a one-time download...'">>"%VBSHELPER%"
echo f.WriteLine "        INFO 'Please wait — this may take 5-15 minutes depending on your internet speed.'">>"%VBSHELPER%"
echo f.WriteLine "        ollama pull llama3.2">>"%VBSHELPER%"
echo f.WriteLine "        if ($LASTEXITCODE -eq 0) { OK 'Llama 3.2 AI model ready!' }">>"%VBSHELPER%"
echo f.WriteLine "        else { WARN 'Model download failed. JARVIS will still work — open Settings to configure AI.' }">>"%VBSHELPER%"
echo f.WriteLine "    } else { OK 'Llama 3.2 already installed!' }">>"%VBSHELPER%"
echo f.WriteLine "    # Add ollama to Windows startup so it auto-starts">>"%VBSHELPER%"
echo f.WriteLine "    $startupDir = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\Startup'">>"%VBSHELPER%"
echo f.WriteLine "    $ollamaStartup = Join-Path $startupDir 'OllamaAutoStart.bat'">>"%VBSHELPER%"
echo f.WriteLine "    if (-not (Test-Path $ollamaStartup)) {">>"%VBSHELPER%"
echo f.WriteLine "        '@echo off' + [char]10 + 'start /min ollama serve' | Out-File $ollamaStartup -Encoding ASCII">>"%VBSHELPER%"
echo f.WriteLine "        OK 'Ollama set to auto-start with Windows!'">>"%VBSHELPER%"
echo f.WriteLine "    }">>"%VBSHELPER%"
echo f.WriteLine "} else { WARN 'Ollama not available. You can install it later from https://ollama.com' }">>"%VBSHELPER%"
echo f.WriteLine "Step 6 'Setting up JARVIS voice...'">>"%VBSHELPER%"
echo f.WriteLine "try {">>"%VBSHELPER%"
...
echo f.WriteLine "Step 7 'Creating shortcuts...'">>"%VBSHELPER%"
...
echo f.WriteLine "Write-Host '   Local AI (Llama 3.2) is ready — no internet needed!' -ForegroundColor Cyan">>"%VBSHELPER%"
