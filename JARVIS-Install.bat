...
echo.>>"%PS%"
echo # Write JARVIS.vbs using ASCII (no BOM) — VBScript requires plain ASCII>>"%PS%"
echo $vbs = Join-Path $INSTALL_DIR 'JARVIS.vbs'>>"%PS%"
echo $electronExe = Join-Path $INSTALL_DIR 'node_modules\.bin\electron.cmd'>>"%PS%"
echo $vbsText = 'Set WS = CreateObject("WScript.Shell")' + Chr(13) + Chr(10)>>"%PS%"
echo $vbsText = $vbsText + 'WS.CurrentDirectory = "' + $INSTALL_DIR + '"' + Chr(13) + Chr(10)>>"%PS%"
echo $vbsText = $vbsText + 'WS.Run "cmd /c node_modules\.bin\electron.cmd . --no-sandbox", 0, False' + Chr(13) + Chr(10)>>"%PS%"
echo $vbsText = $vbsText + 'Set WS = Nothing'>>"%PS%"
echo $asciiBytes = [System.Text.Encoding]::ASCII.GetBytes($vbsText)>>"%PS%"
echo [System.IO.File]::WriteAllBytes($vbs, $asciiBytes)>>"%PS%"
echo.>>"%PS%"
echo # Desktop shortcut>>"%PS%"
